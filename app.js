...
class App {
  ...
  showView(viewId) {
    ...
    // Toggle body login-page class for centering
    document.body.classList.toggle('login-page', viewId === 'login');
  ...
  generateInvoice(txIdOrIds) {
    ...
    const unsettledSection = document.getElementById('inv-unsettled-note-section');
    const unsettledNote = document.getElementById('inv-unsettled-note');
    ...
    const paidStamp = document.getElementById('inv-paid-stamp');
    if (hasUnsettled) {
      ...
      unsettledNote.textContent = `މި ބިލުގައިވާ ފައިސާ މަތީގައިވާ އެކައުންޓަށް ޖަމާކުރުމަށްފަހު ${contact}ށް ސްލިޕް ފޮނުވުންއެދެމެވެ.`;
